...
router.get("/signed_volunteers/:lessonId", async (req, res) => {
  try {
    ...
    const results = await pool.query(
      "SELECT * FROM signed_volunteers  where lesson_id=$1",
      [lesson_id]
    );
    res.status(200).json({
      status: "success",
...
router.post("/signed_volunteers", async (req, res) => {
  try {
    ...
    const results = await pool.query(
      "INSERT INTO signed_volunteers (full_name, email, lesson_id, role_id) VALUES ($1,$2,$3,$4)",
      [fullName, email, lesson_id, role]
    );
    res.end();
  } catch (error) {
    console.log(error.message);
...
router.get("/lesson/delete/:lessonId", async (req, res) => {
  try {
    ...
    const results = await pool.query("DELETE FROM lesson where id=$1", [
      lesson_id,
    ]);
    res.status(200).json({
      status: "success",
